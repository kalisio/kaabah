resource "aws_ebs_volume" "manager_volume" {
  count             = "${var.provider == "AWS" ? var.manager_additional_volume_count * var.manager_instance_count : 0}"
...
resource "null_resource" "manager_volume_mount" {
  ...
  depends_on = ["aws_volume_attachment.manager_volume_attachement"]
}
...
resource "aws_ebs_volume" "worker_volume" {
  ...
  tags {
    Name = "${terraform.workspace}-worker-volume-${count.index}"
  }
}
