resource "null_resource" "services" {
  ...
  provisioner "remote-exec" {
    inline = [
      "bash ${local.tmp_dir}/install-services.sh ${var.domain} ${var.subdomain} ${var.ca_server} ${var.contact} ${var.auth_user} '${var.auth_password}' ${var.docker_network} ${openstack_compute_instance_v2.manager.network.1.fixed_ip_v4} ${var.slack_webhook_url} ${var.slack_channel}",
