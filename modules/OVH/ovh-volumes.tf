...
resource "openstack_compute_volume_attach_v2" "manager_volume_attachement" {
  count         = var.OVH && var.manager_additional_volume_size > 0 ? var.manager_instance_count : 0
  region        = var.region
  instance_id   = openstack_compute_instance_v2.manager.*.id[count.index]
  volume_id     = openstack_blockstorage_volume_v2.manager_volume.*.id[count.index]
}
...
resource "null_resource" "manager_volume_mount" {
  ...
  connection {
    ...
    host                = openstack_compute_instance_v2.manager.*.access_ip_v4[count.index]
    user                = var.ssh_user
...
resource "null_resource" "worker_volume_mount" {
  ...
  connection {
    ...
    bastion_user        = var.bastion_ssh_user
    bastion_private_key = file(var.bastion_ssh_key)
    host                = openstack_compute_instance_v2.worker.*.access_ip_v4[count.index]
    user                = var.ssh_user
    private_key         = file(var.ssh_key)
    timeout             = local.timeout
  }  
