resource "null_resource" "manager_user_script" {
  count = "${var.provider == "OVH" && length(var.manager_user_scripts) > 0 ? var.manager_instance_count : 0}"
...
resource "null_resource" "worker_user_scripts" {
  count = "${var.provider == "OVH" && length(var.worker_user_scripts) > 0 ? var.worker_instance_count : 0}"
